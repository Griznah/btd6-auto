...
def test_set_round_state_fast_success(mock_keyboard):
    ...
    assert (
        vision.set_round_state(
            "fast", find_in_region=mock_find_in_region_factory(1)
        )
        is True
    )
...
def test_set_round_state_slow_failure(mock_keyboard):
    ...
    assert (
        vision.set_round_state(
            "slow", max_retries=2, find_in_region=lambda x: False
        )
        is False
    )
...
def find_in_region(template_path, region=None, threshold=None):
    # Mock implementation of find_in_region for testing
    if region is None and threshold is None:
        ...
        try:
            ...
            try:
                ...
                # If still TypeError, try single-argument call
                if isinstance(result, bool):
                    return result, None
        # result can be bool or tuple
